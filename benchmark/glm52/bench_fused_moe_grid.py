#!/usr/bin/env python3
"""ONE GPU, seconds: measure the grid-size clamp that ``_grid_rows`` applies
to the fp8 fused-MoE Triton GEMMs at GLM-5.2's decode shape.

``_grid_rows`` clamps ``EM`` to ``min(sorted_token_ids.shape[0],
topk_ids.numel() * BLOCK_M)``; without the clamp the worst-case padded slot
count is ``tokens*top_k + E*(BLOCK_M-1)`` = 3,872 at k=3 decode (242
M-tiles), most of which are empty early-exit CTAs.

This benchmark times clamped vs unclamped grids eagerly and inside a CUDA
graph (where they actually run) to confirm the clamp's effect. Run on any
idle GPU:
