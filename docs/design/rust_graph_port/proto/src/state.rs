...
use crate::spec::{Dest, EdgeSpec, GraphRef, LoopId, NodeId, Sym};
use serde::Serialize;
...
impl Slot {
    fn new(n: usize) -> Self {
        Slot { mask: 0, tensors: vec![None; n] }
    }
    fn clear(&mut self) {
        self.mask = 0;
    ...
    fn set(&mut self, slot: u8, t: Vec<TensorRef>) {
        self.mask |= 1 << slot;
...
impl RequestState {
    ...
    pub fn ingest(&mut self, node: NodeId, slot: u8, tensors: Vec<TensorRef>, can_buffer: bool) -> bool {
        let name = self.graph.node(node).inputs[slot as usize];
        {
            let st = &mut self.nodes[node as usize];
            if !st.cur.has(slot) {
                st.cur.set(slot, tensors.clone());
            } else if can_buffer && !st.next.has(slot) {
                st.next.set(slot, tensors.clone());
            } else {
                return false;
            }
        }
    ...
    pub fn input_tensors(&self, node: NodeId, next_iter: bool) -> Vec<(Sym, Vec<TensorRef>)> {
        ...
        spec.inputs.iter().enumerate()
            .filter_map(|(i, &n)| slot.tensors[i].as_ref().map(|t| (n, t.clone())))
            .collect()
    }
    ...
    pub fn complete(&mut self, node: NodeId, out_tensors: &[Vec<TensorRef>]) -> Vec<RoutedEdge> {
        let graph = self.graph.clone();
        ...
        match spec.loop_id {
            ...
            Some(lid) => self.entity_completed(lid, 0, &mut out),
        }
    ...
    fn entity_completed(&mut self, lid: LoopId, edges_from: usize, out: &mut Vec<RoutedEdge>) {
        let graph = self.graph.clone();
    ...
    fn finish_loop(&mut self, lid: LoopId, out: &mut Vec<RoutedEdge>) {
        let graph = self.graph.clone();
        let lspec = graph.lp(lid);
        ...
        match lspec.parent {
            None => self.root_entity_done(),
            Some(p) => self.entity_completed(p, own_edges_from, out),
        }
    }
    ...
    fn advance_loop(&mut self, lid: LoopId, out: &mut Vec<RoutedEdge>) {
        ...
        // Re-inject the loop's saved external inputs. Python returns these as
        // output_edges for the worker to re-route; routing them here skips a
        // boundary round-trip and a redundant fanout for a same-worker edge.
        let ext = self.loops[lid as usize].ext_inputs.clone();
        for (name, dest, t) in ext {
            if let Some(slot) = graph.node(dest).slot_of(name) {
                self.ingest(dest, slot, t.clone(), false);
            }
            out.push(RoutedEdge {
    ...
    pub fn ingest_for_speculation(&mut self, source: NodeId, edges: &[RoutedEdge]) -> Vec<SpecNode> {
        ...
        for e in edges {
            ...
            self.nodes[d as usize].spec.set(slot, e.tensors.clone());
