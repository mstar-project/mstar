...
.. list-table::
   ...
   * - ``KVSpec(config=PagedKVConfig(...))``
     - A paged KV cache. ``KVConfig`` is the abstract base for common model geometry;
       ...
       ``max_num_pages``, ``page_size`` and ``cpu_offload_pages`` (the number of pinned
       host pages used for offload; 0 disables offload), ``prefix_cache`` and
       ``prefix_cache_salt`` fields.
