...
| DiT execution | Integrated | Runtime tables materialize after load; `compile_dit` independently selects compiled or eager denoise/cache regions; optional 128-token and 512-token rollout and prime graphs captured on H100 | Record a full server graph-off run |
