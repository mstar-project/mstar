...
from mstar.model.ming_flash_omni.config import (
    AudioEncoderConfig,
    MingFlashOmniModelConfig,
    ThinkerLLMConfig,
    VisionEncoderConfig,
)
from mstar.model.ming_flash_omni.ming_flash_omni_model import MingFlashOmniModel
...
# ---------------------------------------------------------------------------
# The shipped deploy configs
# ---------------------------------------------------------------------------


@pytest.mark.parametrize(
    "config_name, expect_talker, expect_imagegen",
    [
        ("ming_flash_omni.yaml", True, False),
        ("ming_flash_omni_imagegen.yaml", True, True),
        ("ming_flash_omni_thinker_only.yaml", False, False),
    ],
)
def test_shipped_configs_divide(
    config_name: str, expect_talker: bool, expect_imagegen: bool,
) -> None:
    """The configs in ``configs/`` must name real graph nodes and divide.

    A node name typo, or a node dropped from the graph, only surfaces at server
    launch otherwise — and a TP group naming a non-TP-enabled node is rejected
    at load. Both are cheap to catch here.
    """
    from pathlib import Path

    repo_root = Path(__file__).resolve().parents[2]
    cfg = str(repo_root / "configs" / config_name)

    # A model with every optional stack enabled, so a config declaring a node
    # the model can register is exercised rather than silently skipped.
    from mstar.model.ming_flash_omni.config import ImageGenConfig

    model = _talker_enabled_model()
    model.config.image_gen = ImageGenConfig()
    node_names = _worker_graph_node_names(model.get_worker_graphs(cfg))
    assert "Thinker" in node_names
    assert ("Talker" in node_names) is expect_talker
    assert ("ImageGen" in node_names) is expect_imagegen

    # tp_size groups must only name nodes the model declares TP-capable.
    sharding = model.get_sharding_config(cfg)
    for group in sharding.groups:
        assert group.nodes <= model.get_default_sharding_config().tp_enabled_nodes
