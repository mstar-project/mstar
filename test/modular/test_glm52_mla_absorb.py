...
def test_absorbed_resource_path_matches_naive_and_dense_reference():
    ...
    with torch.no_grad():
        reference = _ref_dense_mla(naive, naive_cfg, h_all, torch.arange(total))
...
def test_absorbed_path_needs_the_processed_weights():
    (_, _), (absorbed, absorbed_cfg) = _build_pair(seed=7)
    fresh = Glm52MLAAttention(absorbed_cfg)
    fresh.load_state_dict(absorbed.state_dict())
