...
def test_postprocess_audio_returns_int16_pcm() -> None:
    """The serving audio contract is headerless int16 PCM (see
    ``mstar.client.types.AudioChunk``), so a float waveform is scaled."""
    ...
    assert np.frombuffer(out, dtype=np.int16).tolist() == [0, 16384, -16384, 32767]


def test_postprocess_audio_passes_through_int16() -> None:
    wav = torch.tensor([0, 16384, -16384], dtype=torch.int16)
    out = _model().postprocess(wav, "audio")
    import numpy as np

    assert np.frombuffer(out, dtype=np.int16).tolist() == [0, 16384, -16384]
