...
``ingest_for_speculation`` API in the exact shape that
``Worker._try_speculate_next`` relies on: one call taking the producer's whole
output list plus its node name, returning the nodes that became ready.
  ...
  - ``clear_speculative_inputs`` resets the slot so the next outer iter's
    ingestion works again — without it the re-ingest trips the duplicate
    assert in ``ReadySignals.update``.
  - On the gate-failure rollback path: partial ingestion does NOT advance the
    gate even when ``ready_signals`` happens to be full from the in-flight
    iter (for a same-node spec the gate reads ``ready_next_iter``, not
    ``ready_signals``).
"""
...
def _ingest_outputs(wgio: WorkerGraphIO, node_name: str) -> list[SpeculativeNodeInfo]:
    """Mirror the worker: hand every output of ``node_name`` to the graph layer
    in one call, and take the ready list from the return value."""
    rid_node = wgio.nodes[node_name]
    return wgio.ingest_for_speculation(rid_node.outputs, rid_node.name)
...
def test_per_rid_speculative_state_is_isolated():
    ...
    spec0 = _ingest_outputs(wgios[0], "ar_decode")
    assert len(spec0) == 1
    ...
    for wgio in wgios[1:]:
        ...
        assert not wgio._nodes_with_speculative_inputs


def test_first_rid_gate_match_shape():
    """The worker inspects the list returned for the first rid for the
    ...
    matches = [
        info for info in _ingest_outputs(wgio, "ar_decode")
...
def test_clear_speculative_inputs_resets_for_next_outer_iter():
    ...
    reset the slot, otherwise the next outer iter re-ingests names that are
    still present and trips the duplicate assert in ``ReadySignals.update``."""
    wgio = _per_rid_wgios(1)[0]
    assert _ingest_outputs(wgio, "ar_decode")  # fires once
    ...
    assert len(_ingest_outputs(wgio, "ar_decode")) == 1
...
def test_gate_fails_when_only_partial_outputs_ingested():
    ...
    # inputs. A same-node spec gates on ready_next_iter | speculative_signals,
    # so a full ready_signals must not be able to satisfy it.
    ...
    ready = wgio.ingest_for_speculation(
...
def test_speculative_signals_sees_tensor_info_via_shared_reference():
    ...
    wgio.ingest_for_speculation([producer_token_edge], "ar_decode")
...
def test_speculative_node_info_carries_loop_name():
    ...
    wgio = _per_rid_wgios(1)[0]
    ready = _ingest_outputs(wgio, "ar_decode")
    assert len(ready) == 1
...
def test_speculative_node_info_loop_name_none_for_top_level():
    ...
    ready = wgio.ingest_for_speculation(
        [GraphEdge(name="x", next_node="solo")], "solo"
    )
...
def test_prefill_to_decode_transition_gate_shape():
    ...
    ``is_new_loop_iter`` is False here: the flag is source-relative, computed
    from ``source_reg.loop._loop_back_inputs``, and prefill lives outside
    ar_loop so it contributes no loop-back edges. Entering the loop is not the
    same event as iterating it — contrast ``test_loop_back_speculation``,
    where ar_decode feeds itself and the flag is True.
    """
    wgio = _per_rid_wgios(1)[0]
    prefill_node = wgio.nodes["prefill"]
    # Ingest every prefill output (the worker hands over ``sample_node.outputs``
    # wholesale, not just the loop-back ones).
    ready = _ingest_outputs(wgio, "prefill")
    assert len(ready) == 1
    assert ready[0].node_name == "ar_decode"
    assert ready[0].is_new_loop_iter is False  # entering ar_loop, not iterating it
    # loop_name is the DESTINATION's enclosing loop, so it is still set.
    assert ready[0].loop_name == "ar_loop"
...
def test_producer_covering_only_some_dest_inputs_does_not_fire():
    """A destination that a producer only partly feeds must never fire.

    The worker hands over every output of the producer, so the safety net is
    the gate itself, not a caller-side filter: ar_decode needs both ``token``
    and ``kv_cache``, and a prefill that emits only ``token`` leaves it short
    no matter how many of prefill's outputs get ingested.
    """
    graph = Sequential(sections=[
        GraphNode(
            name="prefill",
            input_names={"prompt"},
            # Only one of ar_decode's two inputs; kv_cache comes from elsewhere.
    ...
    assert _ingest_outputs(wgio, "prefill") == []
    # The edge did land in the buffer — the gate declined on coverage, not
    # because the ingest was silently dropped.
    assert wgio.nodes["ar_decode"].speculative_signals.ready_names == {"token"}
