...
def test_build_stacked_params_covers_every_expert() -> None:
    """3 rules per expert × num_experts, plus the dense-MLP and QKV rules."""
    rules = _build_thinker_stacked_params(num_experts=8)
    # 3 × 8 expert rules + 2 dense-MLP (gate/up) + 3 QKV (q/k/v) = 29
    assert len(rules) == 3 * 8 + 2 + 3
    assert {r.shard_id for r in rules if r.shard_id in ("q", "k", "v")} == {"q", "k", "v"}
