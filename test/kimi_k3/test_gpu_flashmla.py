...
cache, eager and captured, at Kimi K3's per-rank shape."""
import pytest
import torch
...
pytestmark = pytest.mark.skipif(not (torch.cuda.is_available() and flashmla_available()), reason="needs a Hopper GPU with FlashMLA")
...
def plan_inputs(lengths):
    ...
    last = [n - (p - 1) * PAGE for n, p in zip(lengths, pages)]
...
def test_flashmla_captured_replay_equals_eager():
    ...
    q_nope = torch.randn(rows, H, L, device=DEV).to(torch.bfloat16)
    q_pe = torch.randn(rows, H, R, device=DEV).to(torch.bfloat16)
    fm = FlashMLAWrapper(H, L, R, PAGE, (128 + R) ** -0.5, max_pages_per_row=32, batch_size=rows, device=DEV, use_cuda_graph=True)
    fm.plan(qo, kv_indptr, kv_idx, last, causal=True, dtype=torch.bfloat16)
    ...
    fi = FlashInferMLAWrapper(torch.empty(128 << 20, dtype=torch.uint8, device=DEV), H, L, R, PAGE, (128 + R) ** -0.5, device=DEV)
