...
def test_forget_drops_the_descriptor_too(bk):
    bk.put_tensor(1, _info(1))
    bk.forget_tensor(1)
    assert not bk.is_tracked(1)
    assert bk.get_info(1) is None


def test_collectable_filters_to_the_free_ones(bk):
    for u in (1, 2, 3):
        bk.put_tensor(u, _info(u))
    bk.increment_ref(2, 1)
    bk.set_persist(3, True)
    assert bk.collectable([1, 2, 3, 99]) == [1]
...
def test_update_info_rebinds_without_touching_refcounts(bk):
    # The slice path re-points a descriptor at a fresh uuid mid-flight.
    bk.put_tensor(1, _info(1))
    bk.increment_ref(1, 2)
    bk.update_info(1, _info(1, address=0xBEEF))
    assert bk.get_info(1).address == 0xBEEF
    assert not bk.can_gc(1), "update_info must not reset the refcount"
...
def test_batch_forms_match_the_single_ones(bk):
    ...
    assert bk.is_tracked(1) and bk.is_tracked(2)
    ...
    bk.set_persist_batch([1, 2], True)
    assert not bk.can_gc(1) and not bk.can_gc(2)

    infos = bk.get_info_batch([1, 2, 99])
...
def test_update_info_batch_rebinds_each(bk):
    ...
    bk.update_info_batch(
        [1, 2], [_info(1, address=0xA), _info(2, address=0xB)]
    )
    assert bk.get_info(1).address == 0xA
    assert bk.get_info(2).address == 0xB
...
def test_tensor_store_works_over_either_backend(bk):
    ...
    store.put_tensor(rid=5, uuid=1, tensor=tensor, info=_info(1))
