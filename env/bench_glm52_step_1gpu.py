...
def make_resources(cfg, bs: int, pages: int, page_size: int = 128):
    """The resources the engine builds for GLM-5.2 (glm52_model.get_node_resources):
    a KVLayout.MLA pool [layers+1, pages, page_size, 576] (one extra plane for
    the MTP module), the MLA attention resource over it, a greedy sampler."""
    ...
    from mstar.model.glm52._testing import GreedySampler
    ...
    kv = KVManager(
        cfg=kv_cfg, name=KV_RESOURCE, joint_comm_group=None, device=DEVICE,
    ...
    attn = MlaAttentionManager(
        kv_cache=KV_RESOURCE, device=DEVICE, dtype=torch.bfloat16, kv_config=kv_cfg,
        softmax_scale=cfg.qk_head_dim ** -0.5, ckv_dim=cfg.kv_lora_rank,
    )
    ...
    return resources, runner, rids


def make_runners(sub, resources, runner):
    """One PiecewiseCudaGraphRunner per region the submodule declares,
    captured — what Engine.warmup does for the node."""
    from mstar.engine.cuda_graph_runner import PiecewiseCudaGraphRunner

    configs = sub.get_piecewise_cuda_graph_configs(DEVICE, torch.bfloat16, tp_world_size=1)
    runners = {
        label: PiecewiseCudaGraphRunner(
            label=f"LLM_{label}", config=config, resources=resources, step_runner=runner,
            device=DEVICE, autocast_dtype=torch.bfloat16, joint_comm_group=None,
            node_name="LLM",
        )
        for label, config in configs.items()
    }
    for r in runners.values():
        r.prepare_for_capture()
    captured = {}
    for label, r in runners.items():
        r.warmup_and_capture()
        if r.any_graphs:
            captured[label] = r
    return captured
...
def make_fwd_info(cfg, rid, max_tokens):
    ...
    return SimpleNamespace(
        request_id=rid, graph_walk="decode", max_tokens=max_tokens,
...
def decode_step(sub, ei, infos, rids, nxt):
    ars = [sub.prepare_inputs("decode", infos[rid], {"text_inputs": [nxt[rid]]})
           for rid in rids]
    # under MTP the submodule declares no outer step for decode: the regions
    # declare, plan and commit their own (Engine._drive_step skips the sweep)
...
def run_arm(cfg, model, args, *, use_graphs: bool, steps: int, trace_path=None):
    ...
    resources, step_runner, rids = make_resources(cfg, args.bs, args.pages)
    ...
    try:
        if use_graphs:
            ...
            runners = make_runners(sub, resources, step_runner)
        ...
        ei = ModelInputsFromEngine(
            request_ids=rids, per_request_info=infos, resources=resources,
        ...
        for rid in rids:
            step_runner.remove_request(rid)
        for r in resources.values():
            r.cleanup()
        del runners, resources, step_runner, sub
