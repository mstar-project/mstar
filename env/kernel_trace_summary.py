...
def main() -> None:
    ...
    coll_n = sum(c for _, c, _ in coll)
    coll_t = sum(d for _, _, d in coll)
    ...
    print(f"busy       : {busy/1000:.2f} ms ({100*busy/span:.1f} %)   "
          f"gaps: {(span-busy)/1000:.2f} ms in {len(gaps)} gaps"
    ...
    print(f"kernel dur : p50 {pct(.5):.1f} us  p90 {pct(.9):.1f}  p99 {pct(.99):.1f}  max {durs[-1]:.1f};"
          f" <3us: {sum(d < 3 for d in durs)}  3-10: {sum(3 <= d < 10 for d in durs)}"
          f"  >=10: {sum(d >= 10 for d in durs)}")
