"""The command line of tier 0.

    python -m fuzzer.tier0 list
    python -m fuzzer.tier0 run --seeds 5000
    python -m fuzzer.tier0 run --machine graph_io --time 60 --all
    python -m fuzzer.tier0 run --machine graph_io --save loop-back-requeue
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="python -m fuzzer.tier0")
    ...
    run.add_argument("--seeds", type=int, default=2000)
    run.add_argument("--start", type=int, default=0, help="the first seed")
    run.add_argument("--ops", type=int, default=80, help="the number of ops in a case")
    run.add_argument(
        "--time", type=float,
