...
`mla` — and `AttentionManager.build` reaches them by deferred import, so naming
a backend in a spec does not load the others.
"""
    ...
    def build(cls, spec: AttentionSpec, info: EngineResourceInfo):
        ...
        if backend == AttnBackend.MLA:
            ...
            return MlaAbsorbManager(
                ...
                mla_ckv_dim=spec.config.mla_ckv_dim,
