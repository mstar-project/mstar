...
class LinearAttnBackend(Enum):
    FLASHINFER = "flashinfer"  # GDN: FlashInfer's gdn kernels
    AUTO = "auto"  # KDA: FlashKDA prefill where it fits (head_dim 128, bounded gate), fla otherwise
    FLA = "fla"  # KDA: flash-linear-attention's Triton kernels only
    FLASHKDA = "flashkda"  # KDA: FlashKDA prefill (CUTLASS, sm90a) + fla decode; raises where unavailable
