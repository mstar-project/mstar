...
and betas. The outputs are packed ``prefix + block`` rows for ``kda_recurrent_checkpoint``: the prefix
...
def kda_verify_prep(
    ...
    ``checkpoint_pos [rows]`` int32 (accepted length - 1). Static shapes, device tensors only: capturable."""
    p = h * d
    w = conv_w.shape[1]
