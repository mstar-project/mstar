...
class KVCache:
    ...
    def __init__(
        ...
        if cfg.layout == KVLayout.NHD:
            ...
            # one latent row per token: ckv||kpe, no K/V axis, no head axis
            self.tensor = torch.zeros(
                cfg.num_layers, cfg.max_num_pages,
    ...
    def chunk_ptrs(
        ...
        if self.layout == KVLayout.MLA:
            # one chunk: the latent rows [token_start, token_end) of the page
    ...
    def write_tokens(
        ...
        Under MLA ``k`` is the per-token latent ([num_tokens, head_dim], the
        ckv||kpe row) and ``v`` must be None: the value is a slice of the same
        latent, so there is nothing separate to store."""
        if self.layout == KVLayout.MLA:
            if v is not None:
                raise ValueError(
                    "KVLayout.MLA stores one latent per token; pass v=None"
                )
        ...
        if v is None:
            raise ValueError("KVLayout.NHD needs both k and v")
