...
@torch.compiler.disable
def _latent_scatter_eager(
    cache: torch.Tensor, layer_idx: int, latent: torch.Tensor,
    page_idx: torch.Tensor, cache_idx: torch.Tensor,
) -> None:
    cache[layer_idx][page_idx, cache_idx] = latent.to(cache.dtype)


def page_shape(cfg: KVConfig) -> tuple[int, ...]:
    """One page's shape, past the layer and page axes — the one place a
    layout's geometry is written down (the host mirror reads it too)."""
    if cfg.layout == KVLayout.NHD:
        return (2, cfg.page_size, cfg.num_kv_heads, cfg.head_dim)
    if cfg.layout == KVLayout.MLA:
        return (cfg.page_size, cfg.head_dim)
    raise NotImplementedError(f"KV layout {cfg.layout} is not recognized.")
...
class KVCache:
    ...
    def __init__(
        ...
        self.tensor = torch.zeros(
            cfg.num_layers, cfg.max_num_pages, *page_shape(cfg),
    ...
    def layer_view(self, layer_idx: int) -> torch.Tensor:
        ...
        head_dim]; MLA: [max_num_pages, page_size, ckv + kpe]."""
    ...
    def read_latent(
        ...
        """Gather the slots written by ``write_latent``, as
        [num_tokens, ckv + kpe]. A gather, so a copy rather than a view."""
        if self.layout != KVLayout.MLA:
            raise NotImplementedError(
                f"read_latent is not implemented for layout {self.layout}."
            )
        return self.tensor[layer_idx][page_idx, cache_idx]

    def write_latent(
        self, layer_idx: int, latent: torch.Tensor,
        page_idx: torch.Tensor, cache_idx: torch.Tensor,
        return_tensor: bool=False,
    ) -> torch.Tensor | None:
        """Scatter one compressed latent per token ([num_tokens, ckv + kpe])
        into the (page, offset-in-page) slots given by
        ``page_idx``/``cache_idx``."""
        if self.layout != KVLayout.MLA:
            raise NotImplementedError(
                f"write_latent is not implemented for layout {self.layout}."
            )
        _latent_scatter_eager(
            self.tensor, layer_idx, latent, page_idx, cache_idx,
        )
        if return_tensor:
            return self.read_latent(layer_idx, page_idx, cache_idx)

    def copy_pages(self, src_pages: list[int], dst_pages: list[int]) -> None:
        """Copy whole pages within this cache: ``src_pages[i]`` ->
        ``dst_pages[i]``. Layout-independent — only the leading two axes."""
