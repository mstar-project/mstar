...
class KVPlanOutput:
    ...
    # populated on demand: the packed write addressing builds them, and so does
    # any consumer calling `device_indptrs`
    ...
    def device_indptrs(self, device: torch.device) -> PagedIndptrs:
        """The index tensors on device, built on first use — a decode step's
        write addressing stays on CPU, so nothing else fills them in."""
        if self.cuda_indptrs is None:
            self.cuda_indptrs = self.cpu_indptrs.to_device(device)
        return self.cuda_indptrs
