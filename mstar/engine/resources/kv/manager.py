...
class KVManager(AttentionResource):
    ...
    @torch.compiler.disable
    def read_latent(self, layer_idx: int=None, plan_label: str=None) -> torch.Tensor:
        """The compressed latents this step's plan writes, as
        [num_tokens, ckv + kpe]. MLA layout only."""
        if layer_idx is None:
            layer_idx = self._default_layer_idx
        if plan_label is None:
            plan_label = self._default_label
        plan_state = self._current_plan_states[plan_label]
        n = plan_state.total_tokens
        return self.kv_cache.read_latent(
            layer_idx=layer_idx,
            page_idx=plan_state.token_to_page[:n],
            cache_idx=plan_state.token_to_cache[:n],
        )
    ...
    def write_latent(
        ...
        """`write_kv`'s MLA-layout counterpart: one latent per token, since an
        absorbed-MLA layer has no K/V pair to write."""
        if layer_idx is None:
            layer_idx = self._default_layer_idx
        if label is None:
            label = self._default_label
        plan_state = self._current_plan_states[label]
        n = plan_state.total_tokens
        return self.kv_cache.write_latent(
            layer_idx=layer_idx,
            latent=latent[:n],
            page_idx=plan_state.token_to_page[:n],
            cache_idx=plan_state.token_to_cache[:n],
