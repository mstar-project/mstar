...
class AttentionCallable:
    ...
    def run_mla(
        ...
        """``__call__``'s absorbed-MLA counterpart: the four MLA tensors rather
        than a q/k/v triple, so not the pure-tensor callable Ulysses SP wants.

        Args:
            q_nope: [T, H, L]      query, no-rope part (L = kv_lora_rank).
            q_pe:   [T, H, Drope]  query, rope part.
            kv_c:   [T, 1, L]      compressed KV latent.
            k_pe:   [T, 1, Drope]  rope key.
        Returns:
            [T, H, L] attention output.
        """
        self.kv.write_latent(torch.cat([kv_c, k_pe], dim=-1).squeeze(1))
        return self.attn.run_mla(q_nope, q_pe, self.kv.layer_view())
