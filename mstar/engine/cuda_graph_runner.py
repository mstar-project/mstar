...
class CaptureBlockedError(Exception):
    """A resource this step uses refuses capture. Expected, unlike a failed
    capture, so the caller logs it without a traceback."""


def capture_blocked_by(
    step: SubmoduleStep | None, resources: Mapping[str, Resource],
) -> str | None:
    """Whether a resource this step uses refuses capture. Read off the step,
    not the node: a walk that never touches it stays capturable."""
    if step is None:
        return None
    for key in step.keys():
        resource = resources.get(key)
        reason = None if resource is None else resource.capture_blocked
        if reason is not None:
            return f"resource {key!r}: {reason}"
    return None
...
class CudaGraphRunner:
    ...
    def warmup_and_capture(self):
        ...
        # walks already reported as uncapturable; every bucket of one raises
        blocked_walks: set[str] = set()
        for spec in slot_specs:
            ...
            except CaptureBlockedError as blocked:
                walk = spec.bucket.graph_walk
                if walk not in blocked_walks:
                    blocked_walks.add(walk)
                    logger.info(
                        "Not capturing CUDA graphs for %s walk %r: %s. That "
                        "walk serves eagerly.",
                        self._submodule_name, walk, blocked,
                    )
                continue
            except Exception:
                logger.warning(
    ...
    def _capture_one(self, spec: CGSlotSpec) -> CudaGraphSlot:
        ...
        blocked = capture_blocked_by(step, self._resources)
        if blocked is not None:
            raise CaptureBlockedError(blocked)
        if step is not None:
...
class PiecewiseCudaGraphRunner:
    ...
    def warmup_and_capture(self) -> None:
        ...
        captured: list[bool] = []
        # every shape of a blocked region raises; report it once
            ...
            for slot in range(self._num_slots):
                ...
                try:
                    ...
                    if not blocked_logged:
                        blocked_logged = True
                        logger.info(
                            "PiecewiseCudaGraphRunner[%s]: not capturing: %s. "
                            "This region serves eagerly.", self._label, blocked,
                        )
                except Exception:
                    shape_captured = False
                    logger.warning(
