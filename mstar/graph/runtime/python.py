...
class PythonGraphRuntime(GraphRuntime):
    ...
    def __init__(
        ...
        # registered memory) as it goes, not at request teardown.
        ...
        self._tensor_store = tensor_manager.tensor_store

        # request id string <-> worker-local handle
        self._rids = RidTable()
    ...
    def add_request(
        ...
        # several times for one request; they all share the first's handle.
        handle = self._rids.intern(request_id)
    ...
    def remove_request(
        ...
        if rid not in self._request_info:
        ...
        self._rids.release(rid)

    def get_rid_string(self, handle: int) -> str:
        return self._rids.name(handle)

    def get_rid_handle(self, rid: str) -> int | None:
        return self._rids.handle(rid)
    ...
    def complete_and_route_batch(
        ...
        for i, (rid, wg_id) in enumerate(zip(rids, wg_ids, strict=True)):
            ...
            # SAME tensors, so only the first edge of a signal name counts;
            # counting both would double every token.
            counted: set[str] = set()
            for edge in routing.new_token_outputs:
                if edge.name in counted:
                    continue
    ...
    def _send_worker_graphs_done(
        ...
        rx_info, tx_info, graph_timings = (
            ([], [], {}) if profiling is None else profiling
        )
