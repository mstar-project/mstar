...
from mstar.model.ming_flash_omni.components.model import LingMoeModel
from mstar.model.ming_flash_omni.config import MingFlashOmniModelConfig
from mstar.model.ming_flash_omni.submodules import (
    AudioEncoderSubmodule,
    BailingMoeV2ThinkerSubmodule,
    VisionEncoderSubmodule,
)
from mstar.model.ming_flash_omni.weight_loader import load_thinker_weights
...
class MingFlashOmniModel(Model):
        ...
        **kwargs,
        ...
        if prompt is not None:
            ...
            if "image" in output_modalities and self.config.image_gen is not None:
                from mstar.model.ming_flash_omni.components.prompt_utils import (
    ...
    def postprocess(
        ...
        * ``audio`` — headerless little-endian int16 PCM (Talker AudioVAE
          waveform, mono). That is the serving-wide audio contract:
          ``mstar.client.types.AudioChunk`` reads the stream as int16, and the
          data worker tags each chunk with :meth:`get_output_sample_rate` /
          :meth:`get_output_audio_channels`. A float waveform is clamped to
          [-1, 1] and scaled by 32767; an already-int16 tensor passes through.
        * ``image`` — PNG bytes of the ImageGen RGB output. The pipeline emits a
        ...
        if modality == "audio":
            wav = output.detach().to("cpu")
            if wav.dtype != torch.int16:
                wav = (wav.to(torch.float32).clamp(-1, 1) * 32767).round().to(torch.int16)
    ...
    def get_submodule(
        ...
        # ``sp_group`` is part of the base contract but unused here: no Ming
        # node opts into sequence parallelism (``get_default_sharding_config``
        # leaves ``sp_enabled_nodes`` empty), so the engine never passes a
        # non-trivial group.
    ...
    def _create_vision_encoder_submodule(self, device: str):
        ...
        """
        from mstar.model.ming_flash_omni.components.projectors import (
            MingVisionProjector,
        )
        from mstar.model.ming_flash_omni.components.vision_encoder import (
            build_vision_encoder,
        )
        from mstar.model.ming_flash_omni.weight_loader import (
    ...
    def _create_audio_encoder_submodule(self, device: str):
        ...
        """
        from mstar.model.ming_flash_omni.components.audio_encoder import (
            build_audio_encoder,
        )
        from mstar.model.ming_flash_omni.components.projectors import (
            MingAudioProjector,
    ...
    def _create_talker_submodule(self, device: str):
        ...
        from mstar.model.ming_flash_omni.components.audio_vae import (
            build_audio_vae,
        )
        from mstar.model.ming_flash_omni.components.talker_dit import (
            build_aggregator,
            build_talker_cfm,
            build_talker_heads,
            build_talker_llm,
        )
        from mstar.model.ming_flash_omni.components.talker_generator import (
            TalkerGenerator,
        )
        from mstar.model.ming_flash_omni.submodules import TalkerSubmodule
        from mstar.model.ming_flash_omni.weight_loader import (
    ...
    def _create_imagegen_submodule(self, device: str):
        ...
        from mstar.model.ming_flash_omni.components.imagegen_pipeline import (
            MingImagePipeline,
        )
        from mstar.model.ming_flash_omni.submodules import ImageGenSubmodule
