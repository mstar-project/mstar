...
Native mstar port of `inclusionAI/Ming-flash-omni-2.0`. The punch list below is
kept as the record of how the port was built and what each piece maps to
upstream; the per-step "DONE" markers are historical.

## Status

All steps of the punch list have landed: config, tokenizer/processor, the
Ling-2.0 MoE thinker, the vision/audio encoders, the graph walks + partition
topology, the talker (CFM + AudioVAE) and the image-gen pipeline, plus the
weight loaders for each. `MODEL_REGISTRY["ming_flash_omni"]`, the two deploy
configs and the bring-up scripts under `test/ming_flash_omni/` come with it.

Verification state:

- **CPU / structural** — green. ~350 tests under
  `test/modular/test_ming_flash_omni_*.py` cover the config load, the
  tokenizer/processor wiring, every component forward, the weight remaps, the
  submodule `prepare_inputs → preprocess → forward` contracts, the walk /
  partition derivation and `postprocess`.
- **Live GPU** — the earlier bring-up numbers (TP=8 thinker T2T, talker and
  image-gen smoke) were taken on the pre-rebase branch. They have **not** been
  re-run since the port was rebased onto the current engine; that needs an
  8-GPU box and the ~238 GB checkpoint.

Serving-contract items that changed with the rebase and are worth re-checking
first on a live run: audio output is now emitted as int16 PCM (the engine-wide
convention — `mstar.client.types.AudioChunk` reads int16), `get_submodule` takes
an `sp_group`, and `postprocess` takes `request_kwargs`.

`benchmark/base.py` has `MingFlashOmni` + `ModelType.MING_FLASH_OMNI`, so the
harness drives either the native server (`--inference-system ours`) or a
vllm-omni one (`--inference-system vllm_omni`, see
...
|---|---|---|
| Model class | `qwen3_omni_model.py` (1,529) | `ming_flash_omni_model.py` |
...
1. **Config port — DONE.** `mstar/model/ming_flash_omni/config.py`
...
3. **Ling-2.0 thinker LLM port — IN PROGRESS.**
   ...
   - **3f — DONE** (graph wiring for the text-only generate loop):
     ...
     * `BailingTokenizer` doesn't load under transformers >= 5.0:
       ...
       `ming_flash_omni_model.py`, applied once after the first
...
5. **Thinker graph walks.** `prefill_text`, `prefill_audio`, `prefill_vision`,
   ...
   - **5a — DONE** (`submodules.py`, `ming_flash_omni_model.py`): the two
...
6. **Talker + Audio VAE.** Port `ming_flash_omni_talker.py` + `talker_module.py`
   ...
   - **6e — IN PROGRESS** (Talker submodule + graph walks): split into
     ...
     - **6e-3 — DONE** (graph walks + Thinker→Talker bridge): the
       ...
       Graph + partition additions (all in `ming_flash_omni_model.py`):
...
9. **ImageGen partition.** Separate from the omni pipeline; lives under
   ...
   - **9b — DONE** (modeling + pipeline + wiring): the full image-gen
     ...
     * Wiring (`submodules.py` + `ming_flash_omni_model.py`):
