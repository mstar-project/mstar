    ...
    def forward(self, inputs_embeds: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        if self.blocks is not None:
            for block in self.blocks:
                # HF T5Block returns a tuple whose first two entries are
                # (hidden_states, self-attention position_bias) — a 2-tuple on
                # transformers 4.x (its default is use_cache=False) and a
                # 3-tuple on 5.x, which appends the cross-attention bias. Index
                # instead of unpacking so both shapes work. The first block
                # computes position_bias from its relative-attention table and
                # later blocks reuse it.
                outputs = block(
                    hidden_states,
                    attention_mask=extended_mask,
                    position_bias=position_bias,
                )
                hidden_states, position_bias = outputs[0], outputs[1]
