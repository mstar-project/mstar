...
except Exception:  # pragma: no cover - older diffusers without CP helpers.
...
except ImportError:  # pragma: no cover - dense checkpoint never enters these paths.

    def reorder_tokens_triton_pack(*args, **kwargs):
        raise RuntimeError("LingBot MoE Triton pack kernels are unavailable.")

    def restore_tokens_triton(*args, **kwargs):
        raise RuntimeError("LingBot MoE Triton restore kernels are unavailable.")

    class LightSglangMoeRunnerConfig:  # noqa: D101
        def __init__(self, **kwargs):
            self.kwargs = kwargs

    class LightSglangStandardTopKOutput:  # noqa: D101
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs

    def ensure_sglang_moe_ready():
        raise RuntimeError("LingBot SGLang MoE backend is unavailable.")

    def fp8_scale_from_amax(*args, **kwargs):
        raise RuntimeError("LingBot SGLang MoE backend is unavailable.")

    def quantize_to_fp8_e4m3fn(*args, **kwargs):
        raise RuntimeError("LingBot SGLang MoE backend is unavailable.")

    def sglang_fused_experts(*args, **kwargs):
        raise RuntimeError("LingBot SGLang MoE backend is unavailable.")
...
class LingBotVideoRotaryEmbedding(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor) -> torch.Tensor:
        ...
        needs_rebuild = self.freqs_cis is None or any(m >= l for m, l in zip(max_vals, self.axes_lens, strict=False))
        if needs_rebuild:
...
def _cat_interleave(
    ...
    for x_part, text_part in zip(a_split, b_split, strict=False):
...
class LingBotVideoSparseMoeBlock(nn.Module):
    ...
    def _reorder_tokens(tokens: torch.Tensor, top_scores: torch.Tensor, top_indices: torch.Tensor, num_experts: int):
        ...
        if backend not in {"sort", "argsort", "default"}:
            raise ValueError(f"Unsupported LINGBOT_MOE_REORDER_BACKEND={backend!r}; expected sort or triton_pack")
    ...
    def _restore_tokens_chunked_scatter(
        ...
        for start in range(0, dim, chunk_size):
            ...
            output[:, start:end] = (unsorted.float() * scores_unsorted).sum(dim=1).to(expert_output.dtype)
        return output
