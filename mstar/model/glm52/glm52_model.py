...
class Glm52Model(Model):
    ...
    def get_node_resources(self) -> list[NodeResourceSpec]:
        # Absorbed MLA does not fit the num_kv_heads x head_dim layout: the
        # cache holds one latent row per token per layer (kv_lora_rank + rope
        # dims = 576) shared by all 64 query heads. No Yarn -> the softmax
        # scale is plain qk_head_dim**-0.5. With MTP on, the layer-78 draft
        # module keeps its KV in one extra layer plane at index
        # num_hidden_layers, on the trunk's page table (draft-tail rows are
