...
def ring_memory_bytes(
    ...
    ``num_sessions`` is a straight multiplier: the world dimension is folded into
    ...
    ``num_sessions`` is a deployment sizing knob and the geometry is not."""
    per_slot = 2 * num_sessions * config.n_kv_heads * config.d_head * dtype.itemsize
...
def describe_ring_memory(
    ...
    """
    per_layer = ring_memory_bytes(config, num_sessions=num_sessions, dtype=dtype)
    total = sum(per_layer)

    lines = [
        f"Waypoint ring KV  variant={config.variant}  worlds={num_sessions}  dtype={dtype}  "
    ...
    other_total = sum(ring_memory_bytes(other, num_sessions=num_sessions, dtype=dtype))
