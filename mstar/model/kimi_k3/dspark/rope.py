...
class YarnRotary(nn.Module):
    ...
    def apply(self, x: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        if x.is_cuda and x.stride(-1) == 1 and positions.is_contiguous():
            from mstar.model.kimi_k3.dspark.rope_kernel import yarn_rope_fused

            # one launch, bit-identical to the torch path below; the rope part of a query or key
            # arrives as a split view, which the kernel reads in place
            return yarn_rope_fused(x, self.cos, self.sin, positions)
