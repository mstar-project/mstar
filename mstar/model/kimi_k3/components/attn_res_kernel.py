...
The residual add that precedes a read (``prefix + attention_out`` before the FFN read,
``prefix + ffn_out`` before the next layer's attention read) is folded into pass 1 as well
(``attn_res_add_read_triton``): the prefix candidate is loaded as ``prefix + add`` rounded to
the activation dtype — bit-identical to the eager add — and written out as the new prefix, so
the two adds per layer cost no launch of their own.
"""
...
def attn_res_scores_kernel(
    prefix_ptr, add_ptr, newp_ptr, blocks_ptr, w_ptr, scores_ptr,
    stride_pt, stride_at, stride_nt, stride_bt, stride_bm,
    D, num_blocks, eps,
    BLOCK_D: tl.constexpr, HAS_ADD: tl.constexpr,
):
    ...
    v = tl.load(tl.where(is_prefix, pre_ptr, blk_ptr), mask=mask, other=0.0)
    if HAS_ADD:
        # the prefix candidate is (prefix + add) rounded to the activation dtype, exactly what the
        # eager residual add produces; the sum is also the layer's new prefix. Block candidates
        # add zero and round to themselves.
        a = tl.load(add_ptr + t * stride_at + offs, mask=mask & is_prefix, other=0.0)
        v = (v.to(tl.float32) + a.to(tl.float32)).to(v.dtype)
        tl.store(newp_ptr + t * stride_nt + offs, v, mask=mask & is_prefix)
    v = v.to(tl.float32)
...
def attn_res_mix_kernel(
    ...
    out = acc
    if APPLY_OUT_NORM:
        # the read's result is rounded to the activation dtype (as the unfused read returns it),
        # then KimiRMSNorm: fp32 statistics, normalize, cast, multiply by the weight in the
        # activation dtype -- the same arithmetic as the separate module, bit for bit
...
def _launch(
    ...
    out_eps: float,
    add: torch.Tensor | None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Returns ``(read, prefix)`` where ``prefix`` is ``prefix + add`` (a new tensor) when ``add``
    is given and the input prefix itself otherwise."""
    t, d = prefix.shape
    m = blocks.shape[1] if blocks is not None else 0
    if m == 0 and out_norm_weight is None:
        # softmax over one candidate: the read is the prefix itself
        new_prefix = prefix if add is None else prefix + add
        return new_prefix, new_prefix
    if m == 0:
        blocks = prefix.unsqueeze(1)  # unused (num_blocks=0), keeps strides valid
    assert m + 1 <= MAX_CANDIDATES, m
    out = torch.empty_like(prefix)
    new_prefix = prefix if add is None else torch.empty_like(prefix)
    ...
    attn_res_scores_kernel[(t, m + 1)](
        prefix, prefix if add is None else add, new_prefix, blocks, w, scores,
        prefix.stride(0), 0 if add is None else add.stride(0), new_prefix.stride(0),
        blocks.stride(0), blocks.stride(1),
        d, m, float(eps), BLOCK_D=block_d, HAS_ADD=add is not None, num_warps=num_warps,
    )
    attn_res_mix_kernel[(t,)](
        new_prefix, blocks, scores, out, out_norm_weight if out_norm_weight is not None else w,
        new_prefix.stride(0), blocks.stride(0), blocks.stride(1), out.stride(0),
        d, m, float(out_eps),
...
def attn_res_read_triton(
    ...
    return _launch(prefix, blocks, score_weight, eps, out_norm_weight, out_eps, None)[0]


@torch.compiler.disable
def attn_res_add_read_triton(
    prefix: torch.Tensor,
    add: torch.Tensor,
    blocks: torch.Tensor,
    score_weight: torch.Tensor,
    ...
    """The read of ``prefix + add`` (``add [T, D]`` in the prefix dtype, e.g. the attention or
    FFN output whose residual add is still pending) -> ``(read, prefix + add)``; both are new
    tensors, the sum bit-identical to the eager ``prefix + add``."""
    assert add.shape == prefix.shape and add.dtype == prefix.dtype, (add.shape, add.dtype)
    return _launch(prefix, blocks, score_weight, eps, out_norm_weight, out_eps, add)
