...
class SchedulingType(Enum):
    ...
    # Model-declared (node, graph_walk) ranks. The old engine-type table is
    # gone — every node shares one engine — so only the model knows which
    # walk is latency-sensitive. A busy high-priority walk can starve the rest.
    PRIORITY = "priority"
...
class MicroScheduler:
    ...
    def __init__(
        ...
        node_walk_priority: dict[tuple[str, str], int] | None = None,
    ):
        self.engine_manager = engine_manager
        self.batch_number = 0
        self.sched_type = sched_type
        # Lower value = higher priority. Unlisted (node, walk) pairs are 99.
        self.node_walk_priority = node_walk_priority or {}
    ...
    def _select_node_priority(
        ...
        # Pick the node whose most-common walk has the best (lowest) rank,
        # then take that node's walk. Returning the loop variable instead of
        # best_node_name pairs the last dict key with the winning walk — an
        # empty set — and get_next_batch idles despite ready work.
        best_node_name = None
        ...
        for node_name, reqs in node_name_to_requests.items():
            ...
            prio = self.node_walk_priority.get((node_name, graph_walk), 99)
    ...
    def get_next_batch(
        ...
        if self.sched_type == SchedulingType.PRIORITY:
            best_node_name, graph_walk = self._select_node_priority(
