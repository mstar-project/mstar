...
class RequestStateManager:
    ...
    def add_request(self, rid: int, current_fwd_info: CurrentForwardPassInfo):
        ...
        once per partition for a given rid.
        """
        info = self.per_request_info.setdefault(rid, PerRequestInfo())
    ...
    def update_request_info(
        ...
        info = self.per_request_info.get(rid)
        if info is None:
            return
    ...
    def get_fwd_info(self, rid: int, partition_name: str):
        return self.per_request_info[rid].per_partition_info[
            partition_name
        ].current_fwd_info
