...
fp16 routed-expert dispatch (:func:`fused_experts`) and for fp8-resident
block-scaled experts (:func:`fused_experts_fp8`, GLM-5.2 / GLM-5.3-Flash).
The shared expert (when present) stays outside and the router stays
outside — these functions handle only the gate_up GEMM, SwiGLU, down GEMM,
and weighted sum over top-k.
"""
...
def fused_experts_fp8(
    ...
    w1_fp8 : torch.Tensor
        ...
        from the fp8-resident MoE block
        directly.
    w2_fp8 : torch.Tensor
    ...
    swiglu_limit : float | None
        GLM-5.3's opt-in pre-activation SwiGLU clamp: gate -> max ``L``,
        up -> ``[-L, L]`` before the activation (threaded to
        :func:`act_and_mul_triton`). ``None`` (default) is the clampless
        glm52/qwen path -- a strict no-op.
    ...
    # 4. SwiGLU (with GLM-5.3's opt-in pre-activation clamp) in bf16, then
    # requantize the intermediate for GEMM-2.
    act_and_mul_triton(cache1, cache2, activation=activation, swiglu_limit=swiglu_limit)
