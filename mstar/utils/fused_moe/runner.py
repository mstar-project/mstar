...
fp16 routed-expert dispatch (:func:`fused_experts`) and for GLM-5.2's
fp8-resident experts (:func:`fused_experts_fp8`). The shared expert (when
present) stays outside and the router stays outside — these functions
handle only the gate_up GEMM, SwiGLU, down GEMM, and weighted sum over top-k.
"""
...
def fused_experts_fp8(
    ...
    w1_fp8 : torch.Tensor
        ...
        e4m3 or its uint8 byte view (the model's ``experts.gate_up_proj_fp8``).
    w2_fp8 : torch.Tensor
    ...
    # get_default_config ignores N, so the down GEMM (N=hidden) would inherit
    # the gate/up tiles; at decode shapes each launch gets its own
    # sweep-picked tiles instead (env/bench_fused_moe_config.py).
