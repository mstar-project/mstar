...
# M*: only the MXFP4 (FE2M1f weights, E8M0 scales) x bf16 instantiations are needed for Kimi K3;
# the other weight/activation types would add ~80 s of compile time and 60 MB of binary.
if os.environ.get("MARLIN_MOE_MXFP4_BF16_ONLY"):
    QUANT_CONFIGS = [dict(q, c_type=["kBFloat16"]) for q in QUANT_CONFIGS
                     if q["b_type"] == "kFE2M1f" and q.get("s_type") == "kFE8M0fnu"]
