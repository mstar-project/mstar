...
| TP by symmetric state and a logical clock | Group-minimum of matched length on the rank-0 broadcast | Allocator state is already symmetric across ranks by construction and checked at warmup. Hits and releases ride the same decisions; only wall-clock recency could diverge, which the worker already flags for LRU offload. |
...
| Tail-first by LRU, no explicit rule | Leaf-first tree eviction | Recency is monotone along a chain, so LRU is tail-first for free. A mid-run eviction only strands pages; it cannot corrupt. |
